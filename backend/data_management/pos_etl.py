"""
POS → DW ETL: Extract data from pos_system, Transform, Load into retails_dataset.

Maps POS normalized tables to DW star-schema:
  - sales_orders (InStore) + sales_order_items → FactSales
  - sales_orders (Online)  + sales_order_items → FactOnlineSales
...
def sync_pos_to_dw() -> dict:
    ...
    # ── 0. Check if any changes since last ETL ────────────────
    ...
    # ── 2. Transform all rows into batch lists ────────────────
    ...
    # ── 3. Batch load into DW (much faster than row-by-row) ──
    BATCH_SIZE = 2000
    ...
    logger.info(
        "POS→DW sync done: %d InStore, %d Online rows loaded.",
...
def _refresh_summary(dw_engine):
    ...
    with dw_engine.begin() as conn:
        # Delete POS-sourced rows (SalesKey >= offset) then re-insert
        conn.execute(text(f"""
        ...
        conn.execute(text(f"""
            ...
            SELECT DATE(f.DateKey), COALESCE(f.StoreKey,0), f.ProductKey,
                   COALESCE(f.PromotionKey,0),
                   SUM(f.SalesQuantity), SUM(f.SalesAmount)
            FROM FactSales f
            WHERE f.SalesKey >= {_SALES_KEY_OFFSET}
            GROUP BY DATE(f.DateKey), COALESCE(f.StoreKey,0), f.ProductKey, COALESCE(f.PromotionKey,0)
        """))
        # Also for online
        conn.execute(text(f"""
            REPLACE INTO summary_daily_sales
            ...
            GROUP BY DATE(f.DateKey), COALESCE(f.StoreKey,0), f.ProductKey, COALESCE(f.PromotionKey,0)
        """))
    logger.info("summary_daily_sales refreshed for POS data.")
