...
# ── Memory-safe configuration for 16 GB RAM machines ──────────────
...
def _load_daily_data() -> pd.DataFrame:
    ...
    Strategy (memory-safe):
      1. Prefer Parquet cache (~190K rows, ~29 MB) — instant & zero MySQL load.
      2. Fallback: build Parquet cache from summary_daily_sales table.
      3. Last resort: load_raw_data() → aggregate (slow, high RAM).
    """
    # ── 1. Try Parquet cache first ────────────────────────────────
    if DAILY_SNAPSHOT_FILE.exists():
        logger.info("Loading data from Parquet cache (memory-safe)…")
        df = pd.read_parquet(DAILY_SNAPSHOT_FILE)
        if len(df) > 0:
            logger.info(f"Parquet cache loaded: {len(df):,} rows, {df['ProductKey'].nunique()} products")
            return df

    # ── 2. Build Parquet cache from DB ────────────────────────────
    logger.info("Parquet cache not found. Building from database…")
    try:
    ...
    # ── 3. Last resort: raw load + aggregate ──────────────────────
    logger.info("Falling back to raw data load (high memory)…")
...
def train_global_model():
    ...
    logger.info("Loading daily sales data…")
    ...
    # 3. Feature engineering — process in chunks to limit peak RAM
    logger.info("Engineering features per product (chunked)…")
    ...
    print(f"Training data: {len(global_df):,} rows × {len(feature_columns)} features")
    ...
    print(f"\n✅ FULL WORKFLOW COMPLETE! Model saved to '{model_path}'")
