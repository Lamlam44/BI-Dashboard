"""
XGBoost Forecasting Model for demand forecasting with quantile regression.
Implements multi-model approach for point forecasts and confidence bounds.
Supports multi-step recursive forecasting.

Migration note (LightGBM → XGBoost):
  - LGBMRegressor replaced by XGBRegressor
  - num_leaves → max_depth (XGBoost uses depth-based tree growth)
  - force_col_wise removed (LightGBM-specific)
  - verbose=-1 → verbosity=0
  - Quantile objective: 'quantile' with alpha → 'reg:quantileerror' with quantile_alpha
  - Model persistence: joblib .pkl → native XGBoost .json
  - Categorical features: XGBoost enable_categorical=True via DMatrix (not needed here
    since all features are already numeric after feature engineering)
"""
...
from modules.demand_forecasting.config import MODEL_PARAMS, TRAIN_TEST_SPLIT
...
class DemandForecastingModel:
    """
    XGBoost model for demand forecasting with quantile regression.
    ...
    def __init__(self, model_params: Dict = None):
        ...
        Args:
            model_params: XGBoost model parameters (uses defaults if None)
        """
    ...
    def train(
        ...
        """
        Train quantile regression models (3 XGBoost models).
        
        Trains three separate models:
        1. Base model: objective='reg:squarederror' for point forecasts
        2. Lower bound: objective='reg:quantileerror', quantile_alpha=0.05
        3. Upper bound: objective='reg:quantileerror', quantile_alpha=0.95
        ...
        try:
            ...
            # Remove quantile_alpha if present from shared params
            base_params.pop("quantile_alpha", None)
            
            self.model_base = xgb.XGBRegressor(**base_params)
            self.model_base.fit(
                X_train, y_train,
    ...
    def save(self, filepath: str):
        """
        Save the 3 XGBoost models (.json) and metadata to disk.
        
        Creates:
          - <filepath>_base.json   (base model)
          - <filepath>_lower.json  (lower quantile model)
          - <filepath>_upper.json  (upper quantile model)
          - <filepath>_meta.json   (feature columns & metadata)
        """
        if not self.is_trained:
            logger.warning("Attempted to save an untrained model.")
            return

        base_path = Path(filepath).with_suffix("")  # strip any existing extension

        # Save each XGBoost model in native JSON format
        self.model_base.save_model(str(base_path) + "_base.json")
        self.model_lower.save_model(str(base_path) + "_lower.json")
        self.model_upper.save_model(str(base_path) + "_upper.json")

        # Save metadata (feature columns, product id)
        meta = {
        ...
        logger.info(f"Model successfully saved to {base_path}_*.json")
    ...
    def load(cls, filepath: str) -> "DemandForecastingModel":
        ...
        Also supports legacy joblib .pkl files for backward compatibility
        during the migration period.
        """
        base_path = Path(filepath).with_suffix("")
        meta_path = str(base_path) + "_meta.json"

        # ── Try new XGBoost JSON format first ─────────────────────
        if Path(meta_path).exists():
            ...
            instance.last_training_product_id = meta.get("last_training_product_id", "GLOBAL")

            instance.model_base = xgb.XGBRegressor()
            instance.model_base.load_model(str(base_path) + "_base.json")

            instance.model_lower = xgb.XGBRegressor()
            instance.model_lower.load_model(str(base_path) + "_lower.json")

            instance.model_upper = xgb.XGBRegressor()
            instance.model_upper.load_model(str(base_path) + "_upper.json")

            instance.is_trained = True
            logger.info(f"Model successfully loaded from {base_path}_*.json")
            return instance

        # ── Fallback: legacy joblib .pkl (LightGBM era) ──────────
        import joblib
        pkl_path = Path(filepath)
        if pkl_path.exists():
            logger.warning("Loading legacy .pkl model — consider retraining with XGBoost.")
            ...
            instance.is_trained = True
            logger.info(f"Legacy model loaded from {pkl_path}")
            return instance

        raise FileNotFoundError(f"No model found at {filepath} (tried .json and .pkl)")
